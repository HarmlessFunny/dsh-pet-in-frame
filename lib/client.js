    ...
    function Pet() {
      ...
      React.useEffect(() => {
        ...
        const timer = setInterval(async () => {
          try {
            ...
            if (!alive) return
            setAction(s.action || 'default')
            if (s.rev !== revRef.current) {
              revRef.current = s.rev
              loadFrames(s.action || 'default')
            }
      ...
      React.useEffect(() => {
        ...
        if (action !== 'default' && action !== 'idle') {
          showBubble(texts[action] || (action === 'error' ? '出错了' : ''))
        }
      }, [action])
      ...
      const onUp = (e) => {
        ...
        if (d && !d.moved) {
          if (bubble) { setBubble(null); clearBubbleTimer() }
          else showBubble(texts[action] || texts.idle || '…')
        }
      }
