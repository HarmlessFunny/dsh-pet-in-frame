...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Poll /state at 250ms: at the old 1s cadence a tool dispatched just
    // after a poll was only seen on the next one, so pose switches lagged
    // ~0.5s on average. 250ms keeps switches near-instant (worst case
    // 250ms) at the cost of a trivial extra local JSON request.
    ...
    if (document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      tag.textContent = [
        ...
        '.dshpet-comp{position:absolute;left:74.24%;top:45.61%;width:15.95%;height:15.95%;z-index:2;pointer-events:none;line-height:0}',
        '.dshpet-comp-img{display:block;width:100%;height:100%;pointer-events:none}',
    ...
    function Pet() {
      ...
      // monotonic poll sequence: drops stale responses when a slower poll
      // resolves after a newer one (possible at the faster poll rate)
      ...
      // bumped when the host's asset scan completes (rev change) so the
      // subagent badge requested before the frames existed gets another try
      const [workTick, setWorkTick] = React.useState(0)
      // command badge: which comp animation is pinned while a shell tool runs
      const [comp, setComp] = React.useState(null) // 'bash_comp' | 'pwsh_comp' | null
      const [compFrames, setCompFrames] = React.useState(null)
      const [compIdx, setCompIdx] = React.useState(0)
      // bumped when the host's asset scan completes (rev change) so a comp
      // badge requested before the frames existed gets another load attempt
      const [compTick, setCompTick] = React.useState(0)
      ...
      // pose bubble text: named entry wins, `error` gets its own message,
      // default/idle/sleep get none, anything else (e.g. per-tool poses with
      // no dedicated text) falls back to a generic working message
      const textOf = (act) => {
        if (!act) return null
        const t = texts[act]
        if (t) return t
        if (act === 'error') return '出错了'
        if (act === 'default' || act === 'idle' || act === 'sleep') return null
        return '工作中…'
      }
      ...
      const toggleBubble = (act) => {
        ...
        if (bubble !== null) {
          ...
          setBubble(textOf(act || action) || '待命中')
          setPinned(true)
        }
      }
      ...
      React.useEffect(() => {
        ...
        const timer = setInterval(async () => {
          try {
            ...
            if (!alive || seq !== pollSeqRef.current) return
            ...
            if (s.rev !== revRef.current) {
              ...
              loadFrames(eff)
              // the frames route may have had nothing (fresh asset scan):
              // retry the pinned comp badge so a long command still gets it
              if (s.comp) setCompTick((t) => t + 1)
              if (s.subagentActive) setWorkTick((t) => t + 1)
            }
      ...
      // Same hardening as the command badge: alive guards stale responses,
      // and a single retry (plus each workTick/rev bump) recovers from a
      // fetch that raced the frames route's asset scan — otherwise the badge
      // stayed missing for the whole subagent run.
      React.useEffect(() => {
        if (!subagentActive) { setWorkFrames(null); return }
        let alive = true
        let retry = null
        const load = () => {
          fetch('/dsh-pet-in-frame/frames/subagent_comp')
            .then((r) => r.json())
            .then((d) => {
              if (!alive) return
              if (d && d.frames) {
                setWorkFrames({ urls: d.frames, delay: d.delay || 500 })
              } else if (retry === null) {
                retry = setTimeout(load, 1500)
              }
            })
            .catch(() => { if (retry === null) retry = setTimeout(load, 1500) })
        }
        load()
        return () => { alive = false; if (retry !== null) clearTimeout(retry) }
      }, [subagentActive, workTick])
      ...
      // command badge: pinned inside the pet image while a shell tool runs
      // (same mechanism as the subagent badge, driven by /state's `comp`).
      // alive guards against a stale response for an older comp overwriting
      // the current badge; a single retry (and each compTick/rev bump)
      // recovers from a fetch that raced the frames route's asset scan.
      React.useEffect(() => {
        if (!comp) { setCompFrames(null); return }
        let alive = true
        let retry = null
        const load = () => {
          fetch(`/dsh-pet-in-frame/frames/${comp}`)
            .then((r) => r.json())
            .then((d) => {
              if (!alive) return
              if (d && d.frames) {
                setCompFrames({ urls: d.frames, delay: d.delay || 500 })
              } else if (retry === null) {
                retry = setTimeout(load, 1500)
              }
            })
      ...
      React.useEffect(() => {
        setCompIdx(0)
        if (!comp || !compFrames || !compFrames.urls || compFrames.urls.length < 2 || !compFrames.delay) return
        const timer = setInterval(() => setCompIdx((i) => (i + 1) % compFrames.urls.length), compFrames.delay)
        return () => clearInterval(timer)
      }, [comp, compFrames])
      ...
      return React.createElement(
        ...
        subagentActive && workFrames && workFrames.urls.length
          ...
          : null,
        comp && compFrames && compFrames.urls.length
          ? React.createElement('div', { className: 'dshpet-comp', title: '命令执行中' },
              React.createElement('img', { className: 'dshpet-comp-img', src: compFrames.urls[compFrames.urls.length > 1 ? compIdx : 0], alt: 'command running', draggable: false }))
          : null,
