  ...
  factory: (require) => {
    ...
    if (document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      tag.textContent = [
        ...
        '.dshpet-working{position:fixed;left:58px;top:397px;z-index:10;pointer-events:none;line-height:0}',
        '.dshpet-working-img{display:block;pointer-events:none}',
    ...
    // Logic 2: fixed-position "subagent working" badge, independent of the pet
    // pose (Logic 1). Shown while /state reports subagentActive, cycling the
    // subagent_working frames at their manifest delay. Top-left at (58, 397).
    function SubagentIndicator() {
      ...
      React.useEffect(() => {
        if (active && frames === null) {
          fetch('/dsh-pet-in-frame/frames/subagent_working')
            .then((r) => r.json())
            .then((d) => { if (d && d.frames) setFrames({ urls: d.frames, delay: d.delay || 500 }) })
            .catch(() => {})
        }
      }, [active, frames])
      ...
      React.useEffect(() => {
        setIdx(0)
        if (!active || !frames || !frames.urls || frames.urls.length < 2 || !frames.delay) return
        const timer = setInterval(() => setIdx((i) => (i + 1) % frames.urls.length), frames.delay)
        return () => clearInterval(timer)
      }, [active, frames])

      if (!active || !frames || !frames.urls.length) return null
      const src = frames.urls[frames.urls.length > 1 ? idx : 0]
      return React.createElement('div', { className: 'dshpet-working', title: '子代理运行中' },
        React.createElement('img', { className: 'dshpet-working-img', src, alt: 'subagent working', draggable: false }))
    }
