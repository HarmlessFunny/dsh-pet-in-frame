...
const TEXTS = {
  ...
  search: '我在查资料…', plan: '我在列计划…', ask: '我在问你…',
  error: '哎哟，出错了', done: '搞定啦',
}
const ACTION_KEYS = ['default', 'think', 'bash', 'edit', 'read', 'search', 'plan', 'ask', 'error', 'done', 'idle', 'sleep']
// Tools whose dispatch legitimately blocks far past the stale-tool fallback:
// a question can sit unanswered for minutes, a shell command can run for
// minutes. tools/result always fires when they settle (answer, cancel, error,
// or completion), so the fallback must not preempt them.
const LONG_WAIT_TOOLS = new Set(['ask_user_question', 'bash', 'pwsh'])
