 ...
 * assets directory for image and manifest changes, and serves five HTTP routes:
 *
 ...
 *   POST /dsh-pet-in-frame/wake           → reset the idle countdown
 *
...
// Every tool is its own pose: the action key is the raw tool name, and
// `assets/manifest.json` maps each tool to its image(s). No tool folding in
// code — shared art is expressed as shared manifest values, not shared keys.
const TEXTS = {
...
const LONG_WAIT_TOOLS = new Set(['ask_user_question', 'bash', 'pwsh', 'subagent', 'subagent_fork', 'send_message', 'workflow', 'job_output'])
// How long the subagent pose flashes on subagent/start and subagent/end.
const SUBAGENT_FLASH_MS = 2000
// The client polls /state every 250ms, so even the fastest tool is seen by
// several polls. The min-show hold is kept as a readable display duration:
// every tool stays on screen ~1.5s after dispatch, not just one poll.
const MIN_TOOL_SHOW_MS = 1500
...
export function apply(ctx, config = {}) {
  ...
  const state = { status: 'idle', action: 'default', tool: null, toolSince: 0, error: null, errorUntil: 0, idleSince: 0, subagentCount: 0, subagentUntil: 0, subagentActiveUntil: 0, permissionSince: 0, planUntil: 0, jobCount: 0, jobComp: null, jobUntil: 0, jobActiveUntil: 0, lastTool: null, toolUntil: 0 }
  ...
  // ---- job lifecycle (background commands) ------------------------------
  // Mirrors the subagent flash/badge pair: a background pwsh/bash dispatch
  // (run_in_background: true) flashes the command pose ~2s and keeps the
  // comp badge pinned while the job runs; jobs.onJobDone settles it (job
  // done → command flash again + badge linger). No job-lifecycle event
  // exists in the harness, so the jobs service listener is the end signal.
  const flashJob = () => {
    state.jobUntil = Date.now() + SUBAGENT_FLASH_MS
    leaveIdle()
    setAction('command')
  }
  ctx.on('tools/execute', (exec, next) => {
    try {
      ...
      if (typeof t === 'string' && t) {
        ...
        state.lastTool = t
        state.toolUntil = Date.now() + MIN_TOOL_SHOW_MS
        leaveIdle()
        // A tool executing during the subagent/job flash cancels the rest of
        // the flash window: the tool pose takes over immediately and the
        // flash does not resume once the tool settles. Pure thinking (no
        // tool) keeps the flash running its full 2s instead.
        const now = Date.now()
        if (now < state.subagentUntil) state.subagentUntil = 0
        if (now < state.jobUntil) state.jobUntil = 0
        // Background shell dispatch: start a tracked job — flash the command
        // pose and remember which comp badge to pin until the job settles.
        const args = exec && exec.arguments
        if ((t === 'pwsh' || t === 'bash') && args && args.run_in_background === true) {
          state.jobCount += 1
          state.jobComp = t === 'pwsh' ? 'pwsh_comp' : 'bash_comp'
          flashJob()
        }
        setAction(t)
      }
  ...
  // ---- goal lifecycle --------------------------------------------------
  // The goal state machine advancing (create / update / activate / complete
  // …) briefly flashes the plan pose so a goal driver working between steps
  // stays visible even when no tool is running. A live tool pose wins over
  // the flash; subagents never own goals, but filter defensively anyway.
  ...
  ctx.on('goal/changed', (payload) => {
    try {
      ...
      const aid = a && a.id
      if (typeof aid === 'string' && aid && subagentIds.has(aid)) return
      state.planUntil = Date.now() + GOAL_FLASH_MS
      leaveIdle()
    } catch (e) { console.error('dsh-pet-in-frame goal', e) }
  })
  ...
  // Job settlement: a background command finished (clean, errored, or
  // killed). Flash the command pose like the subagent end-flash, decrement
  // the tracked count, and linger the comp badge 2s after the last job so
  // badge and pose disappear in sync. Jobs owned by live subagents are
  // ignored (subagent isolation). The jobs service is optional: without it
  // the badge still works for foreground shells, only background tracking
  // is skipped.
  const jobs = ctx.get('jobs')
  if (jobs !== undefined && typeof jobs.onJobDone === 'function') {
    ctx.effect(() => jobs.onJobDone((snapshot, owner) => {
      try {
        const own = owner || (snapshot && snapshot.owner)
        const oid = own && (own.id || own.sessionId || (own.agent && own.agent.id))
        if (typeof oid === 'string' && oid && subagentIds.has(oid)) return
        if (state.jobCount > 0) state.jobCount -= 1
        leaveIdle()
        flashJob()
        if (state.jobCount === 0) state.jobActiveUntil = Date.now() + SUBAGENT_FLASH_MS
      } catch (e) { console.error('dsh-pet-in-frame job done', e) }
    }))
  }
  ...
  // ---- frame resolution ------------------------------------------------
  // Resolution order for any action (a pose or a raw tool name):
  //   1. `manifest[action]` (string or { imgs, delay }), filtered to files
  //      that actually exist in the assets dir
  //   2. filename convention: `<action>.png|jpg|jpeg|gif|webp|svg`, then
  //      `default.<ext>` — covers tools not listed in the manifest
  //   3. `manifest['default']` (animated defaults work too)
  //   4. null → the client shows a placeholder
  const manifestFrames = (key) => {
    const m = manifest && manifest[key]
    if (typeof m === 'string') return { frames: [m], delay: 0 }
    if (m && typeof m === 'object' && Array.isArray(m.imgs)) {
      const list = m.imgs.filter((n) => typeof n === 'string')
      if (list.length) return { frames: list, delay: typeof m.delay === 'number' && m.delay > 0 ? m.delay : 500 }
    }
    return null
  }
  const existingFrames = (spec) => {
    if (!spec) return null
    const existing = spec.frames.filter((n) => imageNames.includes(n))
    return existing.length ? { frames: existing, delay: spec.delay } : null
  }
  const resolveFrames = (action) => {
    const want = action || 'default'
    const fromManifest = existingFrames(manifestFrames(want))
    if (fromManifest) return fromManifest
    for (const candidate of [want, 'default']) {
      for (const ext of EXTS) if (imageNames.includes(candidate + '.' + ext)) return { frames: [candidate + '.' + ext], delay: 0 }
    }
    return existingFrames(manifestFrames('default'))
  }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // Subagent pose is a start/end flash, and the command pose is the same
      // flash for background jobs. During the flash window each pose wins
      // over thinking and idle — a tools/result or agent/status fired right
      // after start/end would otherwise instantly overwrite the flash before
      // the client ever sees it — but a live tool pose beats them: if the
      // main agent moved on to a real tool during the flash, show that tool
      // immediately. Once the window passes, revert to think/idle unless a
      // tool is still holding the pose. Same for the permission pose: while
      // an approval request is pending (permissionSince > 0) it wins over
      // everything except an error, and tools/result clears it when the tool
      // settles.
      const errorActive = action === 'error' && state.errorUntil > now
      const permissionActive = state.permissionSince > 0
      const subagentFlash = now < state.subagentUntil
      const jobFlash = now < state.jobUntil
      if (!errorActive && permissionActive) {
        action = 'permission'
      } else if (!errorActive && !permissionActive && subagentFlash && !state.tool) {
        action = 'subagent'
      } else if (!errorActive && !permissionActive && jobFlash && !state.tool) {
        action = 'command'
      } else if (!errorActive && !permissionActive && state.tool) {
        action = state.tool
      }
      // Minimum tool-pose display: the client polls every 250ms, so a tool
      // that settles quickly would otherwise flash by unseen. Hold the last
      // tool pose until toolUntil so every tool is visible for at least
      // ~1.5s, then fall back to think/idle. Active flashes and a pending
      // approval still win over the hold.
      if (!errorActive && !permissionActive && !subagentFlash && !jobFlash && !state.tool && state.lastTool && now < state.toolUntil) {
        action = state.lastTool
      }
      // goal/changed flash: wins only while nothing else holds the pose.
      if (!errorActive && !permissionActive && !subagentFlash && !jobFlash && !(now < state.toolUntil) && !state.tool && now < state.planUntil) {
        action = 'plan'
      } else if (action === 'subagent' && !state.tool && state.subagentUntil && now > state.subagentUntil) {
        action = state.status === 'running' ? 'think' : 'idle'
      } else if (action === 'command' && !state.tool && state.jobUntil && now > state.jobUntil) {
        action = state.status === 'running' ? 'think' : 'idle'
      }
      if (state.status === 'idle' && !state.tool && action !== 'error' && !permissionActive && !(now < state.subagentUntil) && !(now < state.jobUntil) && !(now < state.toolUntil) && !(now < state.planUntil)) {
        const elapsed = state.idleSince ? now - state.idleSince : 0
        action = elapsed >= 30000 ? 'sleep' : 'idle'
      }
      // Command badge: a foreground shell tool holds its own comp while it
      // runs, and through the same min-show window as the pose (lastTool +
      // toolUntil), so badge and pose always appear and vanish together — a
      // quick command that settles between polls previously showed the
      // command pose but never the badge. Otherwise the comp of the last
      // tracked background job stays pinned while that job runs (plus a 2s
      // linger after the last one settles, in sync with the command
      // end-flash). Subagent shell calls never reach here (filtered in
      // tools/execute / onJobDone).
      const compHold = !errorActive && !permissionActive && !subagentFlash && !jobFlash && !state.tool
        && (state.lastTool === 'bash' || state.lastTool === 'pwsh') && now < state.toolUntil
      const comp = state.tool === 'bash' ? 'bash_comp'
        : state.tool === 'pwsh' ? 'pwsh_comp'
        : compHold ? (state.lastTool === 'bash' ? 'bash_comp' : 'pwsh_comp')
        : ((state.jobCount > 0 || now < state.jobActiveUntil) ? state.jobComp : null)
      json(res, {
        status: state.status,
        action,
        tool: state.tool,
        comp,
        jobActive: state.jobCount > 0 || now < state.jobActiveUntil,
