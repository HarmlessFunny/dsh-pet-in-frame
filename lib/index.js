...
const ACTION_KEYS = ['default', 'think', 'bash', 'edit', 'read', 'search', 'learn', 'plan', 'ask', 'subagent', 'error', 'done', 'idle', 'sleep']
...
export function apply(ctx, config = {}) {
  ...
  const state = { status: 'idle', action: 'default', tool: null, toolSince: 0, error: null, errorUntil: 0, idleSince: 0, subagentCount: 0, subagentUntil: 0 }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // Subagent pose is a start/end flash: revert to think/idle once the
      // flash window passes and no tool is holding the pose.
      if (action === 'subagent' && !state.tool && state.subagentUntil && now > state.subagentUntil) {
      ...
      json(res, {
        ...
        subagentActive: state.subagentCount > 0,
