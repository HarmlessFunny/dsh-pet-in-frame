...
const TOOL_MAP = {
  ...
  interrupt_agent: 'subagent', report: 'subagent',
...
export function apply(ctx, config = {}) {
  ...
  const state = { status: 'idle', action: 'default', tool: null, toolSince: 0, error: null, errorUntil: 0, idleSince: 0, subagentCount: 0, subagentUntil: 0, subagentActiveUntil: 0, permissionSince: 0, planUntil: 0 }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (!errorActive && permissionActive) {
        action = 'permission'
      } else if (!errorActive && !permissionActive && subagentFlash) {
        action = 'subagent'
      }
      // goal/changed flash: wins only while nothing else holds the pose.
      if (!errorActive && !permissionActive && !subagentFlash && !state.tool && now < state.planUntil) {
        action = 'plan'
      } else if (action === 'subagent' && !state.tool && state.subagentUntil && now > state.subagentUntil) {
        action = state.status === 'running' ? 'think' : 'idle'
      }
      if (state.status === 'idle' && !state.tool && action !== 'error' && !permissionActive && !(now < state.subagentUntil) && !(now < state.planUntil)) {
