...
const ACTION_KEYS = ['default', 'think', 'bash', 'edit', 'read', 'search', 'plan', 'error', 'done', 'idle', 'sleep']
...
export function apply(ctx, config = {}) {
  ...
  const state = { status: 'idle', action: 'default', tool: null, toolSince: 0, error: null, errorUntil: 0, idleSince: 0 }
  const setAction = (a) => { state.action = a || 'default' }
  // Track continuous idle: enterIdle() stamps the start of an uninterrupted
  // idle period (30s default -> 30s sleep -> loop); any tool / running /
  // error event leaves idle so the next idle period starts fresh.
  let wasIdle = false
  ...
  // the agent counts as idle from the moment the plugin starts, so the
  // 30s default -> 30s sleep cycle runs even without any agent/status event.
  enterIdle()
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const now = Date.now()
      // Idle cycle: first 30s idle -> default pose, next 30s -> sleep frames,
      // then back to default, looping forever. Any activity overrides it.
      let action = state.action
      if (state.status === 'idle' && !state.tool && action !== 'error') {
        const elapsed = state.idleSince ? now - state.idleSince : 0
        action = Math.floor(elapsed / 30000) % 2 === 1 ? 'sleep' : 'idle'
      }
      json(res, {
        status: state.status,
        action,
